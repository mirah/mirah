...
rem will be converted to to wrong ones, when we un-escape. See JMIRAH-2821.
set _ARGS=%*
...
if ["%_CMP%"] == ["--1.9"] (
  set _CMP=-J-Dmirah.compat.version=MIRAH1_9
  goto :jvmarg
)

if ["%_CMP%"] == ["--1.8"] (
  set _CMP=-J-Dmirah.compat.version=MIRAH1_8
  goto :jvmarg
)
...
rem detect first character is a quote; skip directly to miraharg
rem this avoids a batch syntax error
if "%_CMP1:"=\\%" == "\\" goto miraharg

rem removing quote avoids a batch syntax error
if "%_CMP2:"=\\%" == "-J" goto jvmarg

:miraharg
set _MIRAH_OPTS=%_MIRAH_OPTS% %_CMP%
goto vmoptsNext
